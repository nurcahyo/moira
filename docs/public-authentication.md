...
application UUID through the configured application claim and must contain the
configured subject claim. Trusted JWTs without an application binding may still
authenticate for explicitly authorized admin surfaces, but they cannot create,
discover, or read public application resources. When a consumer key and JWT are
combined, the consumer key supplies the required application binding and a JWT
application claim, when present, must match it.
