...
- A `json_schema` request whose reply is **not JSON** is refused with **422 `structured_output_invalid`** (issue #80). It used to return `200` with `structured_output: null` and the model's prose in the output text, which is the same document a legitimately empty answer produces — so a caller could not tell "the provider did not comply" from "the answer was empty". Moira parses JSON here rather than validating against the schema, so a reply that is valid JSON but violates the schema still succeeds; `null`, `{}` and `[]` all parse and are answers, not failures. The refusal names no provider and never carries the reply's bytes.
