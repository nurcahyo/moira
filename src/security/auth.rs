...
use crate::{
    config::{CallerAuthSettings, JwtAuthSettings},
    error::AppError,
};

use super::ApiKeyHasher;
...
impl AuthService {
    ...
    pub async fn authenticate_caller(
        ...
        if self.caller_settings.dev_trust_headers {
            let actor = Actor {
                ...
                subject: header_string(headers, "x-moira-user-id"),
                tenant_id: header_string(headers, "x-moira-tenant-id"),
                application_id: header_string(headers, "x-moira-application-id"),
                external_user_id: header_string(headers, "x-moira-user-id"),
                external_tenant_id: header_string(headers, "x-moira-tenant-id"),
                ..Actor::default()
            };
    ...
    async fn authenticate_trusted_jwt(
        ...
        let jwks = self.jwks(&issuer_config).await?;
        let jwk = jwks
            .find(&key_id)
    ...
    async fn jwks(&self, issuer: &TrustedIssuerConfig) -> Result<JwkSet, AppError> {
        if let Some(cached) = self.jwks_cache.read().await.get(&issuer.issuer)
...
impl AdminAuthenticator {
    pub fn new(settings: JwtAuthSettings, http: Client) -> Self {
        Self { settings, http }
    }
...
impl CallerAuthenticator {
    pub fn new(settings: CallerAuthSettings, http: Client) -> Self {
        Self { settings, http }
    }
...
async fn validate_static_jwt(
    ...
    let claims = decode::<StaticClaims>(token, &key, &validation)
        .map_err(|err| AppError::Unauthorized(err.to_string()))?
        .claims;
    Ok(actor_from_static_claims(claims))
}

fn actor_from_static_claims(claims: StaticClaims) -> Actor {
...
mod tests {
    ...
    fn actor_claims_merge_scope_formats() {
        ...
        assert_eq!(actor.application_id.as_deref(), Some("app-1"));
