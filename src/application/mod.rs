...
pub use memory_extraction::{
    ...
    parse_candidates, render_transcript,
};
// F30 — `status_for_consent_mode` is deliberately **not** re-exported, and is private to
// `memory_extraction`. It answers the consent question from one of the two consent columns, and
// exporting it made that the path of least resistance for anyone who happened to be holding only
// one. The exported entry point is `effective_extraction_status`, which cannot be called without
// both; the rule itself is `MemoryConsentMode::stricter_of`.
