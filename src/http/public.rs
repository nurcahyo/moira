...
use crate::{
    ...
    error::{AppError, ErrorResponse, current_request_id},
};
...
fn sse_event(result: Result<crate::domain::PublicSseEnvelope, AppError>) -> Event {
    match result {
        ...
        Err(error) => Event::default().event("response.failed").data(
            serde_json::to_string(
                &AppError::Internal(error.to_string()).error_response(current_request_id()),
            )
            .unwrap_or_else(|_| {
                serde_json::json!({
                    "error": {
