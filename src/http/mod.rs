...
mod tests {
    ...
    #[test]
    fn generated_openapi_documents_the_i18n_error_contract() {
        let value = serde_json::to_value(documented_router().into_openapi()).unwrap();
        let schemas = value["components"]["schemas"]
            .as_object()
            .expect("OpenAPI schemas");
        let error_detail = schemas.get("ErrorDetail").expect("ErrorDetail schema");
        let properties = error_detail["properties"]
            .as_object()
            .expect("ErrorDetail properties");
        ...
        assert_eq!(properties["message_key"]["type"], "string");
        assert_eq!(properties["message"]["type"], "string");
        assert!(properties["message_args"].is_object());

        for (path, method, status) in [
            ("/health/ready", "get", "503"),
            ("/api/v1/responses", "post", "401"),
            ("/api/v1/admin/applications", "post", "4XX"),
        ] {
            assert_eq!(
                value["paths"][path][method]["responses"][status]["content"]["application/json"]["schema"]
                    ["$ref"],
                "#/components/schemas/ErrorResponse",
                "unexpected error response schema for {method} {path} {status}"
            );
        }
    }
