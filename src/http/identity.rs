...
#[utoipa::path(
    ...
    params(("Idempotency-Key" = Option<String>, Header, description = "Optional replay key. It protects a REFUSED redemption from being retried into a second attempt, not a successful one: because every check runs before the transactional envelope, a repeat of a redemption that SUCCEEDED finds the invitation already consumed and receives 409 invite_already_consumed rather than a replayed 200. A single-use invitation cannot produce a second grant either way.")),
    responses(
