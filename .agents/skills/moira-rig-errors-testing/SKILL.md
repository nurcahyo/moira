...
`classify_completion_error` (`src/orchestration/runtime_factory.rs:530`), as built:
...
`ExecutionFailure` is **not** an `AppError`. It rides in the `failure: Option<ExecutionFailure>` field of `ExecutionOutcome` (`domain/runtime.rs`), which the execution service returns as `Ok(..)`; `failed_outcome` builds it (`application/execution.rs`). The `AppError` column is the error the non-stream endpoint `POST /api/v1/responses` finally emits via `AppError::coded(failure_http_status(class), failure_code(class), failure.message)` (`application/public.rs`, `AppError::coded` at `src/error.rs:78`). Streaming never produces an `AppError` from a provider failure — the HTTP status is already 200 and the failure arrives as a `response.failed` SSE envelope built by `sse_event` (`http/public.rs`).
...
| Moira `tokio::time::timeout` around the call (the `tokio::select!` on `attempt_timeout` in `execution.rs`) | n/a | `Api{504, provider_timeout}` or `{504, deadline_exceeded}` | `ProviderTimeout` / `DeadlineExceeded` | see override | see override | — |
...
- In practice Moira's own `tokio::time::timeout` on `attempt_timeout` (`execution.rs`) fires first and yields a correct `ProviderTimeout`, because Rig configures no `reqwest` timeout at all — see the `ProviderRuntimePolicyRecord` pitfall.
...
⚠ These are Moira-side or configuration bugs classified as retryable upstream errors. Do not "fix" them by adding more substrings; fix them by failing earlier — `normalize_openai_base_url` already rejects a bad OpenAI-family base URL at build time with `AppError::BadRequest` (`orchestration/provider_url.rs`), which `runtime_handle` (`application/execution.rs`) then remaps to `ProviderConfigurationInvalid`, so a bad base URL never reaches `classify_completion_error` at all.

Note `ProviderAuthenticationFailed` falls through `failure_http_status`'s `_` arm to **502**, not 401/403 (`failure_http_status`, `application/public.rs:2037`). That is intentional: an upstream credential problem is not the caller's authentication problem.
...
- `is_retryable` (`pub`, `controls.rs`): `ProviderTimeout`, `ProviderConnectionFailed`, `ProviderRateLimited`, `ProviderUnavailable`, `ProviderUpstreamError`, `CircuitOpen`, `CapacityExhausted`.
- `is_fallback_eligible` (`pub`, `controls.rs`): the same set **plus** `CredentialNotFound`.
- `is_circuit_failure` (private, `controls.rs`): `ProviderTimeout`, `ProviderConnectionFailed`, `ProviderUnavailable`, **`ProviderInvalidResponse`**, `ProviderUpstreamError`, `ProviderRateLimited`.
...
- The stream drain `execute_rig_stream` (`application/execution.rs`) sets its local `committed = true` on the first *output-bearing* item — text deltas, tool calls — and then forces `retryable = false; fallback_eligible = false` on both exit paths: the idle-timeout branch and the mid-stream `Err` branch. Usage and final-metadata chunks are deliberately **not** output and must not flip it (the same flag gates the TTFT metric). It also flips `EventCollector::mark_output_committed`, read back via `output_committed()`.
- `attempt_timeout_failure(bounded_by_total_deadline, output_committed)` (`execution.rs`), covered by the unit test `timeout_after_stream_output_cannot_retry_or_fallback` in the same file.
...
1. **Provider error text never propagates.** `safe_provider_error_message` emits only class + status. `safe_config_error(provider, err)` (`runtime_factory.rs:526`) emits `"build Rig {provider} client failed: {err}"` — the `{err}` is Rig's `ClientBuilderError`, which contains no credential. Note this string *does* reach the caller: `runtime_handle` (`application/execution.rs`) maps **any** factory `AppError` to `ProviderConfigurationInvalid` with `err.to_string()`, and `AppError::Config`'s `Display` is `"configuration error: {0}"` (`src/error.rs:40`). That covers `AppError::BadRequest` from `normalize_openai_base_url` too. Anything you put in a factory error message is public.
2. **Handles and credentials have hand-written `Debug`.** `RuntimeModelHandle` prints `RuntimeModelHandle::OpenAi(<redacted>)` (`runtime_factory.rs:52-62`); `ResolvedCredential` redacts `secret` and passes `config` through `redact_credential_config` (`domain/runtime.rs:400`, `:612`). Rig's own `Client` `Debug` filters `Authorization` and `*api-key*` headers (`src/client/mod.rs:188-219`) but still prints the base URL — that is not sufficient, keep Moira's impls. Never `#[derive(Debug)]` on a type holding a handle or a secret.
...
All in `tests/execution_lifecycle.rs`; find them by test name, not line number.

- `malformed_response_is_sanitized_and_opens_the_provider_circuit` — `!malformed_failure.message.contains("{not valid JSON")` and `!failure.message.contains("sk-lifecycle-secret")`.
- `public_provider_failure_retains_keyed_i18n_error_contract` — the raw HTTP body contains none of the provider payload, the secret, or private diagnostics, and the full public envelope holds: `code == "provider_invalid_response"`, `message_key == "moira.error.provider_invalid_response"`, `message == "provider request failed (ProviderInvalidResponse)"`, `message_args == {}`, `details == null`.

That last assertion means **the sanitised sentence is the literal public message**, and the same `(status, body)` pair is stored for idempotent replay (`idempotency_record` / `PublicRepository`, `application/public.rs` over `infra/repositories/public.rs`). Failure classification is part of the idempotency contract, not just the response.

`failure_code` (`application/public.rs:2057`) is an exhaustive 28-arm match with **no** `_` arm. Adding an `ExecutionFailureClass` variant breaks the build there on purpose. Do not add a wildcard.

## Observability

`src/orchestration/**` still contains **zero** `tracing::` calls — keep it that way; the factory has no safe field to log that the caller does not already get structurally. Plan 05 added instrumentation one layer out, in `src/application/execution.rs`. Observability at the Rig boundary is carried by structured artefacts plus exactly two spans:

- `RuntimeEventEnvelope` (`domain/runtime.rs:507`) — `request_id`, `execution_id`, monotonic `sequence`, `timestamp`, `event_type`, `payload`.
...
- The only two spans constructed in `src/` are `redacted_request_span` → `http_request` (`src/lib.rs`) and `execution_attempt` (`src/application/execution.rs`). Both are `debug_span!` on target `moira`, and both are audited-safe by construction: `execution_attempt` records only `attempt_id`, `attempt_number`, `execution_id`, `provider_id`, `provider_model_id`, `provider_type` (via `provider_type_label`, shared with the metric label so the two cannot drift), `model_key`, and `stream`; `http_request` records method, `uri.path()` only, version, and `x-request-id`, and **no** header values.
- `src/application/execution.rs` also has three `tracing::error!` sites for persistence/bookkeeping failures. They are the exception, not licence to add more.

If you add `tracing` at this boundary, the allowed field set is exactly the identifier/classification surface above plus `provider_type`, `model_key`, and the upstream HTTP status number. Forbidden as fields or in messages: request or response content, `preamble` / agent-profile instructions, `CompletionRequest.additional_params`, provider response bodies, credential ids' plaintext material, base URLs containing embedded tokens, and anything derived from `provider_response_body()`. Do not `Debug`-format a struct into a span field; enumerate the fields.

**Rig emits its own spans, and they are a leak vector.** `GenericCompletionModel::completion` opens `info_span!(target: "rig::completions", "chat", …, gen_ai.system_instructions = &completion_request.preamble, …)` (`src/providers/openai/completion/mod.rs:1928-1944`; the Anthropic equivalent is `src/providers/anthropic/completion.rs:2462-2467`), and at `TRACE` it logs the entire pretty-printed request body (`:1966-1972`). Moira **does** populate `CompletionRequest.preamble` from the agent profile in `build_completion_request` (`application/execution.rs`), so that field is an internal protected instruction.

The default filter `"moira=info,tower_http=info"` (the `env_filter` default in `config/settings.rs`) is target-scoped and therefore safe — `rig::completions` is not enabled, so no layer ever records the span. Since plan 05 there is a second consumer of that filter: when `telemetry.otel_enabled` is set, `config/telemetry.rs` attaches a `tracing_opentelemetry` layer that bridges **every span the subscriber records** to the OTLP exporter, alongside the `fmt` (or `fmt().json()`) layer. The filter is now the only thing standing between `gen_ai.system_instructions` and a remote collector. The rules follow:

- Never broaden `MOIRA_TELEMETRY__ENV_FILTER` to a bare level such as `debug` or `trace` in any environment that handles real prompts. A bare level enables *every* target, including `rig::completions`.
- Moira's own two spans are `debug_span!`, so a stock deployment that only flips `MOIRA_TELEMETRY__OTEL_ENABLED=true` exports an empty trace stream. The documented fix is the **target-scoped** `MOIRA_TELEMETRY__ENV_FILTER=moira=debug,tower_http=debug` — safe because `rig::*` stays disabled. Never "fix" the empty stream with a bare `debug`.
- `config/telemetry.rs` reads `settings.env_filter` first and only falls back to `EnvFilter::try_from_default_env()` (i.e. `RUST_LOG`) when the configured string **fails to parse** — so a malformed `env_filter` silently hands filtering to the ambient `RUST_LOG`. Treat an unparseable filter as a security bug, not a cosmetic one.
- If Rig diagnostics are needed, add an explicit target directive at a safe level (`rig::completions=warn`), never a global level.
- Do not add a subscriber layer, or promote either Moira span to `info_span!`, without re-auditing this. The OTel layer additionally carries a `filter_fn` that drops `opentelemetry`-targeted events to avoid export feedback loops; keep it.
...
**L3 — the scripted OpenAI-compatible server.** `tests/support/mock_openai.rs` is an Axum server bound to `127.0.0.1:0` (`:175`) serving `POST /v1/chat/completions` (`:173`), with `base_url()` returning `http://{addr}/v1` (`:195`). Scripts are a `VecDeque<ProviderScript>` popped per request; every request is recorded as `RecordedRequest { authorization: Option<String>, body: Value }`. Variants: `Completion`, `HeldCompletion`, `HttpError { status, body }`, `MalformedResponse` (a 200 with `Content-Type: application/json` and the body `{not valid JSON`, `:268`), `Stream`, `HeldStream`, `StreamErrorAfterDelta`, `StreamErrorAfterToolCall`, `StalledStream`. An exhausted queue falls back to a 500. Deterministic sequencing uses `ScriptGate` (`wait_arrived` / `release` / `wait_completed` / `wait_connection_closed`, 5s `WAIT_TIMEOUT` at `:32`); `ConnectionGuard` signals abnormal client disconnects. `call_count()` (`:212`) is the assertion that a failure did *not* reach the provider.
...
**L4 — the Postgres lifecycle fixture.** `tests/support/mod.rs::LifecycleFixture::new() -> Option<Self>` (`:137`) requires `MOIRA_TEST_DATABASE_URL`; it returns `None` and the test returns early when unset, but **panics when unset under `CI=true`** (`tests/support/mod.rs:741-750`). `add_provider` drives the real admin API end to end (provider → model `test-model` → credential `sk-lifecycle-secret` → runtime policy → routing policy). Use L4 only for properties that need persistence, credential encryption, circuit state, or capacity accounting.
...
L3 — assert base-URL normalisation and the credential over real HTTP. `normalize_openai_base_url` has direct unit coverage in its own module (`orchestration/provider_url.rs`, test `normalizes_vllm_base_url_to_openai_v1`: `http://192.168.1.13:8000` → `.../v1`, and `http://localhost:8000/v1/` → unchanged, no doubled `/v1`). The end-to-end proof is that the mock server, which only routes `POST /v1/chat/completions`, is reached at all from a `base_url()` of `http://{addr}/v1` — `completion_uses_real_rig_protocol_and_encrypted_credential` (`tests/execution_lifecycle.rs:48`) asserts `Authorization == "Bearer sk-lifecycle-secret"`, `body["model"] == "test-model"`, and `body["stream"] != true`.

For a sanitisation-plus-breaker property, follow `malformed_response_is_sanitized_and_opens_the_provider_circuit` (`tests/execution_lifecycle.rs:691`): script one `MalformedResponse` with `circuit_failure_threshold: 1`, assert `ProviderInvalidResponse` and that the message excludes the raw body, then assert the *next* execution is `CircuitOpen` and `provider.call_count().await == 1`. Note how that class is reached: the mock returns **200** with `Content-Type: application/json` and the body `{not valid JSON` (`tests/support/mock_openai.rs:268`), so `reqwest` passes it through, Rig's decoder fails, and `CompletionError::JsonError` takes the `json` substring branch.
...
- Async unit tests need `#[tokio::test]`. `.next()` on a stream needs `futures_util::StreamExt` in scope, and `use super::*` in `runtime_factory.rs`'s test module (`:595`) already supplies it from the file's own imports — the existing `semantic_stream_preserves_item_order_and_in_band_failures` calls `items.next().await` with no explicit `StreamExt` import. Do not add one.
...
- **`Usage` uses `0` as the "not reported" sentinel**, and `usage()` on a stream is zero until the final response arrives (`src/streaming.rs:307-315`). `usage_from_rig` encodes this via `Usage::has_values()` (`src/completion/request.rs:570`) plus the per-field `non_zero` helper (`runtime_factory.rs:509`); a failed attempt is always persisted with `UsageSummary::default()` (`application/execution.rs`).
- **`CompletionResponse::message_id` is `None`** for OpenAI Chat Completions (`src/providers/openai/completion/mod.rs:1196`), Anthropic, Gemini, DeepSeek, and Azure. `output_from_response` maps it straight to `provider_request_id` (`runtime_factory.rs:477`), so do not write a test asserting a non-null `provider_request_id` on those paths.
- **`ProviderRuntimePolicyRecord` is ignored by the factory** (`runtime_factory.rs:90`, `_policy`), and Rig sets **no** `reqwest` timeout of its own. There is therefore no transport-level deadline at all: timeouts are enforced only by Moira's `tokio::time::timeout` wrappers, all in `application/execution.rs` — `attempt_timeout` per attempt, `idle_timeout` in `execute_rig_stream`, and `send_timeout` in `push_stream` for event delivery. A "connect timeout" test must exercise Moira's wrapper, not a Rig client setting.
...
- **The public message string is frozen** by `public_provider_failure_retains_keyed_i18n_error_contract` (`tests/execution_lifecycle.rs`) and by stored idempotency replays. Reformatting `safe_provider_error_message` is a breaking API change and invalidates persisted replay bodies.
