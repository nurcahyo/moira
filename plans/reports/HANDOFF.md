...
**The expiry arrived, and the split paid.** The maintainer took the decision fail-closed on
2026-08-06 (issue #79). The `documents_` case was replaced by a guard asserting the refusal, and
**all four observability guards next to it stayed green through the change** — which is the whole
claim this entry makes, now measured rather than predicted. One `guards_` case did have to change
and it is worth knowing why: the streaming twin asserted `"stream": true` on the body that reached
the provider, as proof it had not silently taken the non-streaming path. Fail-closed means no body
reaches the provider, so that evidence was destroyed by the fix. **A guard can survive a decision
and still lose its control.** When it does, the replacement control has to be an independent
