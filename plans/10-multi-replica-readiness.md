...
- **Branch:** `plan/10-multi-replica-readiness` — branched from the **current `develop`** (§1 rule 1 and §1A: `develop` is the integration branch and the base for all plan work), never from another plan branch. This plan is **post-MVP**; it has no hard dependency on plans 02–09 landing first, but in practice it is sequenced after them (see `01-roadmap-and-dependencies.md`). If the coordinator elects to stack it on an unmerged base branch, the PR description **must name the base PR** and the branch must be rebased once that base merges (§1.1). Never force-push this branch if another plan stacks on it (§1.7).
