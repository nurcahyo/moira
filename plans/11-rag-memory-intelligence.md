...
## §0 — Wave 0: drift against the tree (re-audit 2026-07-30, HEAD `671eadf`, branch `main`)

**Read this section before any other. The body of this plan was written against a much older tree;
plans 02a, 02b, 03, 04, 05, 06/06b/06c, 07 and 10 have all merged since.** A citation-by-citation
re-audit of the ~110 `file:line` references in the body found **roughly 85 stale** — including the
whole of the body's own "Confirmed still accurate" list (§0.4).

The rule from plans 06 and 07 applies again: **where §0 and the body disagree, §0 wins.** The body is
left in place because its *design* — the sub-phase decomposition, the SQL-level isolation
requirement, the prompt-injection structural-separation rule, the stacked-PR shape — is still sound.
It is the citations and the tree-state assumptions that rotted.

Three body edits have been applied inline rather than only corrected here, because leaving them would
have produced wrong code rather than a wrong reference: every `/v1/…` route prefix is now
`/api/v1/…`; the Definition-of-Done ingestion path is now the real one; and the three false claims
about the test tree, the support-module size and the catalog key count are gone.

### §0.1 Blockers — these break the build, fail a committed test, or corrupt state

| # | Body says | Reality | Required change |
|---|---|---|---|
| **B1** | Ingestion progresses `pending→chunking→embedding→indexed` (`:108`, DoD). § Verification claims *"No conversation/memory/RAG test file exists today."* | **A committed 27.9 KB suite asserts the exact opposite.** `tests/rag_ingestion_honesty.rs` pins `"pending"` at **`:328, 338, 363, 372, 413, 455, 545, 575, 585, 668`**, and `reindex_supersedes_the_previous_version_without_ever_writing_indexed` (`:518`) asserts at **`:589-593`** that `indexed_version_count == 0` — *"no `rag_document_versions` row for this document may ever hold `'indexed'`"*. The file anticipates this plan in a comment at **`:566-573`** (*"Plan 11 is expected to change this line when a real pipeline lands"*), but the plan never mentions the file. `tests/rag_idempotency_replay.rs` (52.2 KB) exercises the same ingest/reindex surface. | **Update the pins in the same PR that lands the pipeline (PR 11.AB) — do not delete either file.** Each `"pending"` assertion becomes an assertion on the *new* honest terminal state; the `indexed_version_count == 0` assertion inverts. The comment at `:566-573` explains why a naive `count(*) = 0` check is laundered by the supersession `CASE` (`src/infra/repositories/conversation.rs:1651`) — the replacement assertion must not repeat that mistake. A PR that changes ingestion without touching these two files is red on `cargo test` and must not be opened. |
| **B2** | `chunk_hash` reuses `crate::security::request_hash`, *"already used identically for `content_hash` elsewhere, e.g. `src/application/conversation.rs:286,352,395,455`"* (`:203`), pinned by a test at `:363`. | **Zero such call sites remain.** Content hashing in that file moved to `self.state.idempotency_hasher.hash(...)` at **`:407, 475, 518, 580, 682`** → `IdempotencyHasher::hash` (`src/security/idempotency.rs:84-90`), which emits `"{pepper_version}:{base64url(hmac_sha256(pepper, bytes))}"` — **keyed and version-prefixed**. `request_hash` itself still exists (`src/security/masking.rs:10`, a plain unkeyed SHA-256 aliasing `secret_fingerprint`) but is re-exported at **`src/security/mod.rs:21`**, not `:14` as `:363` states. The premise that the two functions are interchangeable is false. | **Decision D-B2 below.** Keep `crate::security::request_hash`, delete the false justification, fix the re-export line, and record the consequence D-B2 uncovers for Sub-Phase F. |
| **B3** | A new `moira:diagnostics:read` scope gates the two diagnostic endpoints (`:54`, `:162`, `:329`, DoD checklist). | **Three diagnose scopes already exist in `ADMIN_SCOPES`** (`src/security/authz.rs`): `"moira:runtime:diagnose"` (`:58`), `"moira:memory:diagnose"` (`:98`), `"moira:retrieval:diagnose"` (`:99`). `"moira:memory:extract"` (`:97`) exists too, which Sub-Phase F needs. Being in `ADMIN_SCOPES` already makes them admin-plane-only, which is the entire property the plan wanted the new scope for. This is the same failure mode as plan 10 claiming two i18n keys were missing when both were present. | **Drop `moira:diagnostics:read` everywhere.** Gate `GET …/context-plans/{execution_id}` on `moira:retrieval:diagnose` and `GET …/retrieval-runs` on `moira:retrieval:diagnose`; use `moira:memory:diagnose` for any memory-specific diagnostic surface. The DoD bullet *"the new `moira:diagnostics:read` scope is an authorization change"* becomes *"no new scope is added; the deny-by-default negative test runs against the existing `moira:retrieval:diagnose`"*. |

#### Decision D-B2 — `rag_chunks.chunk_hash` uses `crate::security::request_hash` (unkeyed SHA-256)

**Decided, not deferred.** Both candidates were read in full.

- `IdempotencyHasher::hash` (`src/security/idempotency.rs:84-90`) exists for a specific reason spelled
  out in its own module doc (`:1-13`): idempotency ledger values are digests **over request bodies
  that can carry provider API keys**, so after a database-only compromise an unkeyed SHA-256 lets an
  attacker offline-verify guesses of the secret-bearing body. Peppering closes that.
- That rationale **does not transfer to a chunk hash**, and the mechanism actively breaks the
  requirement. `chunk_hash` must be **content-addressable**: identical text re-ingested must produce
  an identical value, indefinitely, so no-op re-ingestion is detectable. `IdempotencyHasher` is
  keyed and version-prefixed and — by deliberate design documented at `src/security/idempotency.rs:24-40`
  — **verifies only against the currently active pepper**. A pepper rotation therefore silently
  invalidates every stored `chunk_hash`, and every document re-chunks and re-embeds on next ingest.
  That is a correctness and cost regression, not a security gain.
- RAG chunk text is user content, not credential material, and `chunk_hash` is never returned on any
  public surface (`PublicCitation` has no hash field).

**Chosen:** `crate::security::request_hash` (`src/security/masking.rs:10`, re-exported
`src/security/mod.rs:21`), over the exact chunk text bytes. Note in-code that the name is a
misnomer here — it is an alias for `secret_fingerprint`, and the value being hashed is a document
chunk, not a request.

**Reversal condition:** switch to a keyed hasher if `chunk_hash` ever becomes reachable across a
trust boundary — specifically, if it is (a) returned on any caller-visible response, (b) accepted as
a caller-supplied lookup key, or (c) used to dedupe *across* applications/tenants, at which point an
attacker who can submit content and observe dedupe behaviour gains an existence oracle over another
tenant's corpus. Within-scope dedupe (the only kind this plan proposes) does not trip this.

**Consequence D-B2 uncovered, which the audit did not flag:** `memory_records.content_hash` is
*already* written with the keyed hasher (`src/application/conversation.rs:580`, and `:475/:518` for
message content). Sub-Phase F's *"Dedupe: exact match via `content_hash`"* (`:241`) therefore
inherits the rotation fragility above — after a pepper rotation, exact-match memory dedupe silently
...
|---|---|
| `ingest_rag_document` hardcodes `ingestion_status = 'indexed'` at `src/infra/repositories/conversation.rs:1184` (the P0-1 root cause, `:62`, `:108`, `:400`, `:559`) | **Fixed.** Both insert sites bind `RagIngestionStatus::Pending` (`:1597`, `:1677`), with `migrations/0009_backfill_false_indexed_ingestion_status.sql` already shipped to correct historical rows. The only surviving `'indexed'` literal is the supersession `CASE` at `:1651`, which is now unreachable. **Rewrite the P0-1 finding:** the residual defect is not a false status, it is that **ingestion stops at `pending` and never writes `rag_chunks` or `rag_chunk_embeddings`.** The plan's job is to give `pending` somewhere to go. |
| `PgConversationRepository` is a concrete struct; *"P2-3 notes this repo currently lacks a trait; plan 06 addresses trait coverage — add a trait if plan 06 has already landed"* (`:101`) | **`ConversationRepository` is a trait** (`src/infra/repositories/conversation.rs:281`) with **two** implementations: `PgConversationRepository` (`:98`, impl `:509`) and `InMemoryConversationRepository` (`:2017`, impl `:2062`). **Every method this plan adds lands in three places, not one** — the trait declaration and both impls. Sub-Phases A, B, C and E each budget for one. Re-estimate. The in-memory impl must not silently no-op a retrieval method, or unit tests built on it will assert nothing. |
| Sub-Phase F *"must add minimal structured-output support to the completion call path… if none exists yet"* (`:239`) | **Structured output already exists.** `src/application/execution.rs` threads `output_schema` (`:1793-1824`) and `structured_output` (`:714`, `:1443`), and the catalog already carries `moira.error.structured_output_invalid` (`errors.rs:325`) and `..._unsupported` (`:330`) — the plan cites `errors.rs:249-258`, which is stale. Sub-Phase F **wires** structured output; it does not add it. |
| Sub-Phase E/F job bodies plug into workers plan 10 will register | **All four worker names are already registered and metric-seeded** in `WORKER_JOB_NAMES` (`src/infra/workers.rs:26-35`): `memory-extraction-retry`, `conversation-summarization-retry`, `embedding-retry`, `document-ingestion-retry`. `src/infra/metrics.rs` seeds one zero-valued series per entry per counter family (`:291-294`), pinned by `worker_job_names_match_the_spec_table` (`src/infra/workers.rs:389`). The enqueue path validates against the same table. **Do not add names; bind to the existing four.** |
| `tests/` has no conversation/memory/RAG file; `tests/support/mod.rs` is 496 lines; the content-leak suite is entirely missing (`00-audit-report.md` P1-10) (`:344`, `:428`, `:570`, `:572`) | **31 integration files exist**, including `rag_ingestion_honesty.rs` and `rag_idempotency_replay.rs` (B1). `tests/support/mod.rs` is **1112 lines** and `tests/support/mock_openai.rs` is 507. The content-leak suite exists in **two** files — `tests/content_leak_snapshots.rs` (28.1 KB) and `tests/secret_leak_snapshots.rs` (47.2 KB). **Extend those two rather than inventing `tests/rag_content_leak.rs` as a third**; a third file duplicates the canary-token harness and the two existing ones already own the "does user content reach logs/audit" question. |
| The catalog has 61 keys (`:256`) | **116** — 111 in `src/i18n/catalog/errors.rs`, 5 in `src/i18n/catalog/notices.rs`. Corrected inline. `is_known_key` is at `src/i18n/catalog/mod.rs:40` and `default_message_for_key` at `:44`, not `:30-38`. |
| The doc deliverables are net-new files (`docs/pgvector-benchmarks.md`, a new Embeddings section) | **18 plan-11-domain docs already exist** under `docs/`: `context-budgeting.md`, `context-planning.md`, `conversation-summarization.md`, `document-chunking.md`, `document-ingestion.md`, `embeddings.md`, `memory-architecture.md`, `memory-consent.md`, `memory-correction-and-deletion.md`, `memory-extraction.md`, `memory-policy.md`, `memory-retrieval.md`, `pgvector.md`, `rag-architecture.md`, `rag-collections.md`, `rag-security.md`, `retrieval-citations.md`, `retrieval-ranking.md` (plus `conversation-memory-rag-api.md`). **Retarget every doc deliverable at these.** In particular the `EXPLAIN ANALYZE` findings belong in the existing `docs/pgvector.md`, not a new `docs/pgvector-benchmarks.md`. `docs/rig-integration.md` genuinely has no embeddings section — that one deliverable stands. |

### §0.3 Required additions the body lacks entirely

| # | Item | Why it is binding |
|---|---|---|
| **A1** | **Migration numbering starts at `0016`.** | The highest committed migration is **`0015_worker_jobs.sql`**; plan 10 merged `0014_cluster_replica_leases` and `0015_worker_jobs` at `671eadf`. The audit that produced this section said `0013` — it predated that merge. Verified by directory listing. The append-only window is `0001-0015`. If the embedding-dimension Open Decision produces a migration, it is `0016_…`. |
| **A2** | **`POST /api/v1/admin/rag-documents/{id}/reindex` is a second ingestion entry point and the body never mentions it.** | It is registered at `src/http/mod.rs:465` (`conversation::reindex_rag_document`), pinned in the route inventory at `:689` and in the idempotency inventory at `:1256`, backed by `ingest_rag_document_with_connection` (`src/infra/repositories/conversation.rs:1620`) — **the same function `/ingest` calls**. It is also the exact path B1's failing test exercises. **Sub-Phases A and B must wire it identically to `/ingest`**, and the Sub-Phase H idempotency audit list must include it. A pipeline wired only into `/ingest` leaves `/reindex` writing bare `pending` versions forever. |
| **A3** | **Route-group choice is load-bearing.** | `admin_conversation_routes()` (`src/http/mod.rs:427-465`) carries `ADMIN_BODY_LIMIT_BYTES` (2 MiB, `:73`); `conversation_routes()` (`:372`) carries `CONVERSATION_BODY_LIMIT_BYTES` (1 MiB, `:51`). `src/http/mod.rs:400-426` records this exact bug being found and fixed once already — admin RAG paths silently inherited the 1 MiB cap purely because of which Rust module their handlers lived in, so a 1536 KiB body got `400` on one admin path and `413` on three others. **`POST /api/v1/conversations/{id}/summarize` belongs in `conversation_routes()`** (caller-plane, 1 MiB); **the two diagnostic `GET`s belong in `admin_conversation_routes()`** (2 MiB, admin-gated). Registering either in the wrong group re-opens a closed bug. |
| **A4** | **A Metrics section. The plan defines seven observability surfaces and contains zero metrics content.** | Contract, from `src/infra/metrics.rs`: a counter needs (i) a private name const `const X_TOTAL: &str = "moira_…_total";` (pattern at `:78`, `:93`), (ii) a `describe_counter!` inside `describe_and_seed_families` (`:170`), **and** (iii) a zero-seed `counter!(X_TOTAL).increment(0)` — pinned by `the_new_families_are_seeded_at_zero_before_any_observation` (`:1234`). **Histograms are deliberately not seeded.** **Cardinality is enforced**: `ALLOWED_LABEL_KEYS` (`:777-795`) admits only `service, status_class, table, route, method, provider_type, outcome, model_key, state, job_name, operation, channel, le, quantile`, checked by the test at `:1054`. **No `application_id`, `conversation_id`, `collection_id`, `memory_type`, `tenant_id` or `user_id` may ever be a label** — the module doc at `:32-43` states the rule and its reason (unbounded labels are a scrape-path memory-exhaustion vector). Per-application retrieval diagnostics go to **`retrieval_runs` rows**, which is what that table is for. Suggested families, all label-free or closed-label: `moira_rag_chunks_written_total`, `moira_rag_embeddings_written_total`, `moira_rag_ingestion_runs_total{outcome}`, `moira_retrieval_runs_total{outcome}`, `moira_memory_extraction_runs_total{outcome}`, `moira_summarization_runs_total{outcome}`, plus histograms `moira_retrieval_latency_seconds` and `moira_embedding_batch_latency_seconds`. `outcome` must reuse the existing closed set, not a new string. |
| **A5** | **A NOTIFY / circuit-breaker obligation.** | If this plan attaches `notify_moira_runtime_config_change()` to **any** table, it must add that table to `CIRCUIT_UNAFFECTED_RESOURCE_TYPES` (`src/infra/db.rs:264-284`) **in the same change**, with a test. The `other =>` arm at `:343-347` returns `CircuitResetScope::All` **plus a `warn!` per write** — so every write silently discards provider circuit-breaker state on every replica. This is exactly plan 07's B3. **`rag_document_versions` is currently unclassified**, and so are `rag_chunks`, `rag_chunk_embeddings`, `memory_embeddings`, `conversation_summaries`, `context_plans`, `retrieval_runs`, `rag_ingestion_runs` and `memory_extraction_runs`. `rag_collections`, `rag_documents`, `memory_records` and `conversations` **are** classified. If the plan adds no trigger, say so explicitly rather than leaving it unstated. |
| **A6** | **pgvector has no Rust-side binding. Put it in Wave 0's spike scope.** | `Cargo.toml` declares **no `pgvector` crate**, and there is **no `Vec<f32>` anywhere in `src/`**. The `vector(1536)` columns exist in SQL and are never read or written from Rust today. Binding a `Vec<f32>` through sqlx needs either a crate addition (`pgvector` with the `sqlx` feature, which touches `deny.toml` and the supply-chain gate `tests/supply_chain_policy.rs`) or manual text/binary encoding. **The plan budgets neither.** Wave 0 must resolve this alongside the Rig embedding API question, and must state which route it took and what it costs — a new dependency is a reviewable decision on this repo, not an implementation detail. |
| **A7** | **Advisory-lock key collision guard.** | Sub-Phase E's per-conversation singleflight lock needs a key that collides with nothing. `src/infra/workers/leader.rs` declares `LEADER_LOCK_KEYS` (`:39-45`) with exactly **one** entry (`RETENTION_CLEANUP_LEADER_LOCK_KEY`, `moiralrt`), and `the_leader_lock_key_collides_with_no_existing_key` (`:398-407`) asserts it against **four** further taken keys — `moirastp`, `moiraoid`, `moiratdb`, and the test-only `MOIRARET` from `tests/retention_worker.rs`. Five distinct keys are in play, one declared. Sub-Phase E's key must be added to that assertion list, and — because a per-conversation lock is *derived* from a conversation id rather than fixed — it must be namespaced so no derivation can land on any of the five. `declared_leader_lock_keys_are_unique` (`:412`) covers only the declared table. The existing idempotency precedent is `pg_try_advisory_xact_lock` at `src/infra/repositories/admin.rs:659` (the body cites `:567`, stale). |
| **A8** | **Bounded-delete guidance (forward-looking; the plan proposes none today).** | If any sub-phase adds a bounded delete — a chunk-supersession sweep, a context-plan retention pass — it must use `with victims as materialized (…)`. `src/infra/workers/retention.rs:270`, `:344-346` documents why, and `:565-575` is a test asserting the literal token `as materialized` is present in the SQL. The file says in terms: *do not "simplify" these back to `id in (select …)"`. |

### §0.4 The body's own "verified" section is mostly false

The body ends (§ Re-audit corrections applied) with **"Confirmed still accurate (spot-checked, no
drift)"** over ~30 citations. **Stating it plainly: almost none of them are.** A false confidence
marker is worse than no marker, because it is precisely the signal that stops the next reader
checking.

| Body's "confirmed" claim | Reality |
|---|---|
| `ingest_rag_document` at `src/infra/repositories/conversation.rs:1138-1211`, `'indexed'` at `:1184`, tx `:1144`, supersession `:1166-1176`, commit `:1209` | **All stale, and `'indexed'` is gone entirely.** Trait decl `:494`, `PgConversationRepository` impl `:1472`, the real body in `ingest_rag_document_with_connection` `:1620`, `InMemoryConversationRepository` impl `:2330`. Pending binds at `:1597`, `:1677`; supersession `CASE` `:1651`. |
| `citations: Vec::new()` at `src/application/public.rs:1657`; `public_response_from_record` `:1614-1662` | **Stale.** `:1716` and `:1672`. (The *facts* still hold — citations are still unconditionally empty.) |
| `prepare_response_conversation` `:314-381`, `record_assistant_response` `:383-421`, `patch_memory` `:528-565`, `ingest_rag_document` service `:947-979`, archived check `:343-349`, `estimate_tokens` `:1196-1198`, unit test `:1217-1224` | **All stale.** `:437`, `:506`, `:664`, `:1158`, (archived check now inside `:437`+), `:1512`, `:1709`. The file grew by ~500 lines. |
| `ContextPlanner` struct `:34`, `impl :36-49`, `deterministic_phase_five_order` `:37-48` | **Stale.** Struct `:122`, fn `:125`. |
| `conversation_access`/`can_read_all` at `:1040-1065` | **Stale.** `conversation_access` `:1356`, `can_read_all` `:1378`. |
| `put_embedding_policy` `:721-747` | **Stale.** `:857`. |
| *"all seven `request_hash` call sites (`286,352,395,455,546,876,967`)"* | **False — there are none.** See B2. |
| *"all four policy `invalidate_all()` lines (`615,654,695,736`)"* | **Stale.** `:751, 790, 831, 872`. |
| *"all four `Idempotency-Key` declarations at `src/http/conversation.rs:665,847,954,984`"* | **Stale.** `:673, 857, 987, 1019`. |
| `pg_try_advisory_xact_lock` at `src/infra/repositories/admin.rs:567` / `:567-581` | **Stale.** `:659`. |
| `controls.rs:20` imports `RuntimeModelHandle` | **Stale.** `src/orchestration/controls.rs:23`. `runtime_factory.rs` does exist. |
| `PublicCitation` at `src/domain/conversation.rs:258-267`; `PublicResponse.citations` at `src/domain/public.rs:167`; `RagDocumentIngestRequest` at `:611-617` | **Off by one to three.** `:259`, `:170`, `:612`. Substantially true; the *shapes* are unchanged and the "no span fields" claim holds. |
| `src/error.rs:53-65` envelope; derivation rule `:146-148` | **True.** `ErrorResponse` `:53`, `ErrorDetail` `:58`, `format!("moira.error.{}", self.code())` `:147`. |
| `crate::security::request_hash` at `src/security/masking.rs:10` | **True.** Re-export is `src/security/mod.rs:21`, not `:14`. |
| *"every cited line in `migrations/0007_conversations_memory_rag.sql`"* | **True.** Verified `:286-296`, `:382-383`, `:402`, `:404-405`, `:419-429`, `:431`; file is 566 lines. **This and the two rows above are the only survivors of the "confirmed" list.** |
| `docs/rig-integration.md` — 32 lines, one `#` heading, zero `embed`/`Embed`, provider header `:16`, bullets `:18-24` (body `:212-213`) | **True, and the audit was wrong to imply otherwise.** Re-verified line by line. Sub-Phase B's characterisation of the doc gap stands. |
| `docs/todo.md` Phase 5 mapping `:64-82` (`:65-84`) | **Uniformly off by one.** The Phase 5 heading is `:63` and the items run `:65-83`. Correct mapping: planner `:65`, safe injection `:66`, summarization `:67`, extraction `:68`, memory embeddings `:69`, semantic memory retrieval `:70`, retrieval service `:71`, chunking `:72`, finish direct-text ingestion `:73`, remote-URL SSRF `:74`, Rig embedding `:75`, RAG hybrid retrieval `:76`, citations `:77`, MVP-scope doc `:78`, tokenizer-aware budgeting `:79`, export/deletion `:80`, `If-Match` `:81`, tests `:82`, pgvector benchmarks `:83`. |

### §0.5 Citation staleness by file — assume every line number is wrong until re-checked

| File | Status |
|---|---|
| `src/application/conversation.rs` | **All ~15 cites stale**, most by 300-500 lines. Anchors: `ContextPlanner` `:122`, `deterministic_phase_five_order` `:125`, `prepare_response_conversation` `:437`, `record_assistant_response` `:506`, `patch_memory` `:664`, `put_embedding_policy` `:857`, `ingest_rag_document` `:1158`, `conversation_access` `:1356`, `can_read_all` `:1378`, `estimate_tokens` `:1512`, `context_planner_order_keeps_required_content_first` `:1709`, `invalidate_all()` `:751/790/831/872`, `idempotency_hasher.hash` `:407/475/518/580/682`. |
| `src/infra/repositories/conversation.rs` | **All stale, and the file was restructured into a trait.** `PgConversationRepository` `:98`, `trait ConversationRepository` `:281`, Pg impl `:509`, `create_rag_document_with_connection` `:1538`, `ingest_rag_document_with_connection` `:1620`, `InMemoryConversationRepository` `:2017` / impl `:2062`. |
| `src/application/public.rs` | **All stale.** `public_response_from_record` `:1672`, `citations: Vec::new()` `:1716`. |
| `src/security/authz.rs` | **The scope this plan wants to add already exists three times over.** `ADMIN_SCOPE` `:7`, `ADMIN_SCOPES` `:8`, `moira:runtime:diagnose` `:58`, `moira:conversations:write` `:75`, `moira:memory:extract` `:97`, `moira:memory:diagnose` `:98`, `moira:retrieval:diagnose` `:99`. |
| `src/http/mod.rs` | Not cited by the body at all, which is itself the defect (A2, A3). Anchors: `CONVERSATION_BODY_LIMIT_BYTES` `:51`, `ADMIN_BODY_LIMIT_BYTES` `:73`, `conversation_routes` `:372`, the body-limit bug record `:400-426`, `admin_conversation_routes` `:427`, `reindex_rag_document` `:465`, route inventory `:629-689`. |
| `src/infra/db.rs`, `src/infra/metrics.rs`, `src/infra/workers.rs`, `src/infra/workers/leader.rs`, `src/infra/workers/retention.rs` | Not cited by the body at all. See A4, A5, A7, A8 for anchors. |
| `src/i18n/catalog/*` | All stale. `is_known_key` `mod.rs:40`, `default_message_for_key` `mod.rs:44`, `structured_output_invalid` `errors.rs:325`, `structured_output_unsupported` `errors.rs:330`. Every other `errors.rs:NN-NN` in the i18n table must be re-checked before it is quoted in a PR description. |
| `src/http/conversation.rs` | Stale by ~10-35 lines. `Idempotency-Key` declarations `:673, 857, 987, 1019`. |
| `src/domain/conversation.rs`, `src/domain/public.rs`, `src/error.rs`, `src/security/masking.rs` | **Substantially true**, one-to-three-line offsets. See §0.4. |
| `migrations/0007_conversations_memory_rag.sql` | **Every cite true.** The one file the body may still be trusted on. |
| `docs/rig-integration.md` | **True** (`:16`, `:18-24`, 32 lines, no embeddings section). |
| `docs/todo.md` | **Uniformly off by one.** See §0.4. |
...
**User-visible outcome.** `POST /api/v1/responses` calls against a conversation with memory/RAG enabled actually inject relevant history, summaries, memories, and retrieved document chunks into the model's context, and the response's `citations` field is populated with real provenance instead of always being empty. RAG document ingestion actually chunks and embeds content instead of only storing raw text with a hardcoded `'indexed'` status. Conversations past the configured message/token budget get real, versioned summaries instead of unbounded raw history.
...
| **11.E** | `plan/11-e-summarization` | 11.D | E (Wave 5) | Summarization + `POST /api/v1/conversations/{id}/summarize`. |
...
### Worked example — a `/api/v1/responses` call that actually uses retrieval

Request: an application with `memory_retrieval_enabled=true` and `rag_retrieval_enabled=true` calls `POST /api/v1/responses` with `conversation.id` set to an existing conversation and a new user turn asking about a topic covered by a previously-ingested RAG document and a stored preference memory.
...
- **Retrieval query fails or times out**: per `application_embedding_policies.failure_behavior` (migration 0007:109, already defined, default `'continue_without_semantic_retrieval'`) — the context planner proceeds without memory/RAG content rather than failing the whole `/api/v1/responses` call; this must be the default and must be tested explicitly (a broken vector index must never take down the execution path).
...
- New `src/orchestration/embedding.rs`: a thin wrapper analogous to how `src/orchestration/runtime_factory.rs` (referenced by `RuntimeModelHandle` in `controls.rs:20`) wraps completion — construct an embedding-capable Rig client from the resolved provider/model/credential (reuse the exact same credential-resolution path already used for completion, `resolve_runtime_credential` in `src/infra/repositories/runtime.rs`, per the audit's "Credential precedence" positive finding — the audit cited `resolver.rs:254-268`, which plan 06 module 9 deleted as dead code; the live scope-precedence ladder — explicit id, then user, application, tenant, global — has always been the repository one), call Rig's embedding API in batches bounded by `application_embedding_policies.batch_size` (migration 0007:104, default 32), and return `Vec<EmbeddingResult { input_index: usize, vector: Vec<f32>, model_dimension: usize }>`.
...
- New endpoint (`POST /api/v1/conversations/{id}/summarize`, see Interfaces) wired through `src/http/conversation.rs`.
...
**Verified state of the real catalog** (116 keys — 111 in `errors.rs`, 5 in `notices.rs`). Several keys this plan needs **already exist and must be reused, not duplicated**:
...
**Environment facts (verified):** CI runs `pgvector/pgvector:pg16` (`.github/workflows/ci.yml:13-25`) and `migrations/0001_extensions.sql:2` does `create extension if not exists vector`, so pgvector is genuinely available to tests. **`tests/` holds 31 integration files, two of which are already in this plan's domain** (`rag_ingestion_honesty.rs`, `rag_idempotency_replay.rs`) — see §0.1 B1, which is binding over anything this section says about the test tree. `tests/support/mod.rs` still needs new fixture helpers (seeded collections/documents/memories, a deterministic stub embedding provider) before the first new e2e can be written, and the existing `RagFixture` in `rag_ingestion_honesty.rs` is the starting point rather than a blank page.
...
- **`tests/rag_retrieval_end_to_end.rs`** (new, PR 11.C/11.G) — the full chain the plan exists to deliver:
  - `ingest_then_respond_populates_citations_from_real_provenance` — the headline e2e: ingest a document, issue `POST /api/v1/responses` against a conversation with retrieval enabled, assert `citations` is **non-empty** and its IDs match the `context_plans` row's `included_chunk_ids`.
...
- **`tests/rag_content_leak.rs`** (new, PR 11.D — **required, and distinct from the isolation suite**) — proves retrieved text never escapes into observability surfaces. This closes the gap the audit flagged as entirely missing (`00-audit-report.md` P1-10: "no prompt/content-leak suite at all"):
  ...
  - `retrieved_content_never_appears_in_captured_log_output` — capture the `tracing` subscriber output for a full retrieval-backed `/api/v1/responses` call and assert no seeded canary string appears at any level.
...
> ⚠️ **This whole section is superseded by §0.4 and §0.5.** Its closing "Confirmed still accurate
> (spot-checked, no drift)" claim is false for almost every citation it covers. Do not quote anything
> from here in a PR description without re-checking it against the tree first.
...
1. ~~**No test infrastructure exists for this plan's entire domain.**~~ **Superseded by §0.1 B1 and §0.2.** Two RAG e2e files already exist (`tests/rag_ingestion_honesty.rs`, `tests/rag_idempotency_replay.rs`) with a working `RagFixture`. The residual true part: `tests/support/mod.rs` still has no fixtures for memories or embeddings, and harness work remains a **prerequisite deliverable of PR 11.AB**.
