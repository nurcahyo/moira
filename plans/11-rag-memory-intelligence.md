...
**Fail-closed rule:** DB-dependent tests must `panic!` when `CI` is set and `MOIRA_TEST_DATABASE_URL` is absent — the existing verified pattern at `tests/support/mod.rs:430-441`. Never silently skip in CI.
...
- [ ] DB-dependent tests **fail closed in CI** — `panic!` when `CI` is set and `MOIRA_TEST_DATABASE_URL` is absent, matching the verified pattern at `tests/support/mod.rs:430-441`.
