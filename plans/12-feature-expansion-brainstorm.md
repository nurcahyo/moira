...
|---|---------|-------------------|-------------|------------|
| 1 | Claude via subscription OAuth | Reuse existing `credential_type = 'oauth2'` path + rig-core's `anthropic_beta("oauth-2025-04-20")`; console PKCE flow; new `oauth-token-refresh` worker. Ship **opt-in, labeled as against Anthropic's ToS** | MVP (opt-in) | 08 (console), existing credential machinery |
...
graph TD
    ...
    B["B Claude subscription OAuth"]
    C["C ChatGPT OAuth (spike-gated)"]
    D["D Context router"]
...
| Provider | Option A: official API key | Option B: subscription OAuth | Option C: hybrid |
|---|---|---|---|
| **Claude** | Sanctioned, zero ambiguity, works today via existing `'anthropic'` + `ApiKey`. | ToS §3.7 explicitly forbids third-party use of Claude Free/Pro/Max OAuth tokens ("not permitted... in any other product, tool, or service"); Anthropic began enforcing this at the API layer since ~Jan 2026 (non-Claude-Code traffic patterns rejected). Mechanically feasible (rig-core already supports the exact `anthropic-beta` header) but a **known, currently-enforced** violation. | Offer OAuth in the console but label it clearly as unsupported/against Anthropic's terms, off by default, opt-in per deployment. |
| **ChatGPT** | Sanctioned via `api.openai.com`, already representable as `ProviderType::OpenAi`. | No explicit "third-party tool" clause like Anthropic's, but general ToS treats ChatGPT subscriptions as personal/single-user; the OSS ecosystem (opencode-openai-codex-auth) self-restricts to personal use. Also **structurally harder**: `chatgpt.com/backend-api/codex` is a bespoke, reverse-engineered wire format with no rig-core provider — building a client for it means hand-rolling a non-Rig completion path, which is exactly the "parallel LLM abstraction" `CLAUDE.md` forbids. | Same opt-in/labeled pattern as Claude, but gated further behind a research spike proving the wire format is even matchable to `rig_core::completion::CompletionModel` without violating the Moira/Rig boundary. |
| **DeepSeek** | Sanctioned, plain API key, no OAuth concept exists at the platform at all. | N/A — does not exist. | N/A. |

**Recommendation:** ship DeepSeek and Claude/ChatGPT-via-API-key now (DeepSeek needs no code, Claude/ChatGPT already work through the existing `ApiKey` paths). Build the Claude OAuth *plumbing* (credential storage, refresh worker, console flow, `anthropic-beta` header) as an explicitly labeled, opt-in, off-by-default "unsupported / against Anthropic's terms" feature — the marginal engineering cost is low because nearly everything it needs already exists in the schema and in rig-core. Do **not** build ChatGPT OAuth execution in this phase; it is both the higher ToS risk and the one with no clean implementation path today. Do not send Claude-Code-mimicking fingerprint headers (`x-app: cli`, `user-agent: claude-cli/...`) if OAuth is built — impersonating the client is a second, worse violation on top of token reuse, and is plausibly the exact signal Anthropic's enforcement keys on.
...
2. Anthropic OAuth: loosen `require_credential_type` for the `Anthropic` arm, add the conditional `anthropic_beta("oauth-2025-04-20")` call, ship behind an explicit opt-in/unsupported label.
3. Console PKCE flow for Claude only (`start`/`callback` routes), writing through the existing credential-create endpoint.
4. Build the real `oauth-token-refresh` `JobDispatcher`, generic over `credential_type = 'oauth2'` (not Claude-specific), flip its `enabled_by_default` to `true`.
...
- `provider_model_latency_stats` populated from real attempt durations (needs a background aggregation job, not request-path code — another `StubJobDispatcher`-style worker per the plan 11 pattern)
...
- `credential_id` reuses `provider_credentials` — same MOE1 envelope, AES-256-GCM, AAD binding, `masked_secret`/`secret_fingerprint` response contract — rather than inventing a second secret store. Whether that reuse is clean or needs a sibling table is decision 21.
...
|---|------|----------|----------------|
| 1 | Providers | Ship Claude subscription OAuth at all, given Anthropic's active Jan-2026 API-layer enforcement — and if so, opt-in/labeled vs default-on? **This is a legal/ToS decision, not only engineering (see R1/R2), and should be re-surfaced as its own explicit sign-off before any code lands.** | Yes, but opt-in, off by default, clearly labeled as against Anthropic's terms |
...
- **R1 — ToS violation is the headline risk, not a footnote.** Workstreams B and C directly contradict the research digest's own recommendation: Anthropic actively enforces at the API layer (~Jan 2026, non-Claude-Code traffic rejected) and Anthropic's Consumer Terms explicitly name third-party token use as prohibited; OpenAI's ecosystem self-restricts subscription-token reuse outside the official CLI, and third-party tools (e.g. OpenCode) have dropped support after legal requests. Building this per the owner's ask means shipping a feature with demonstrated production breakage and legal exposure. This must be re-surfaced to the owner as its own explicit decision (item 1) before any code lands, independent of this brainstorm doc.
- **R2 — Enforcement is live and can tighten further without notice**, breaking the Claude-OAuth path silently. The failure mode (silent request rejection) must surface clearly rather than look like a generic provider outage. Related: `anthropic-beta: oauth-2025-04-20` is an unversioned magic string owned by Anthropic — nothing at compile time catches a change; only a runtime health check would.
...
- Not a softening of the ToS position: §1's framing of subscription OAuth as a currently-enforced violation of Anthropic's terms, and the equivalent caution for ChatGPT, is the honest state of the world as of 2026-08-14 and must be carried forward verbatim into any implementation plan derived from this one.
