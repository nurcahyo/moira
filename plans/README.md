...
5. **Plan 11 Wave 0 is a blocking research spike** — the Rig embedding API is unverified (`rig-core 0.40.0` resolved; `docs/rig-integration.md` has zero embedding references). If no supported provider exposes embeddings, plan 11 must be re-scoped; Moira does not build a second execution engine.
