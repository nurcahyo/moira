...
### Recommended: a CI guard for `main ⊆ develop` (proposed, not implemented)
...
`push: [main]` is the load-bearing trigger, because `main` advancing is the *only* event that can break the invariant — a push to `develop` can only extend `develop`, which preserves or restores it, and `allow_force_pushes: false` on both branches rules out the reset case. Triggering on `develop` instead would fire on the one event structurally incapable of breaking what is being checked. The schedule catches a merge whose workflow run was skipped or cancelled; keep it, but do not rely on it as the primary — a hotfix on `main` and the release that reverts it can be minutes apart, and "daily" is longer than that window.
...
This is a recommendation only. Adding the workflow is a change to CI configuration and is deliberately out of scope for the change that wrote this text.
