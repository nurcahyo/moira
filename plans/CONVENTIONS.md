...
- **Client secrets are secrets**: stored encrypted using Moira's existing `SecretCipher` (AES-256-GCM + AAD) exactly like provider credentials — never in plaintext, never returned to the browser.
